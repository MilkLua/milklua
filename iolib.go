...
func fileCloseAux(L *LState, file *lFile) int {
	...
	case lFileProcess:
		...
		if e2, ok := err.(*exec.ExitError); ok {
			if s, ok2 := e2.Sys().(syscall.WaitStatus); ok2 {
...
errreturn:
	L.RaiseError("error closing %s: %s", file.Name(), err.Error())
	return 0
}
...
func fileLinesIter(L *LState) int {
	...
	if err != nil {
		...
		L.RaiseError("error reading %s: %s", file.Name(), err.Error())
	}
	L.Push(LString(string(buf)))
	return 1
}
...
func ioInput(L *LState) int {
	...
	case LString:
		...
		if err != nil {
			L.RaiseError("error opening %s: %s", lv, err.Error())
		}
