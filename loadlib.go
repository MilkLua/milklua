...
func loFindFile(L *LState, name, pname string) (string, string) {
	...
	lv := L.GetField(L.GetField(L.Get(EnvironIndex), "pkglib"), pname)
	path, ok := lv.(LString)
	if !ok {
		L.RaiseError("pkglib.%s must be a string", pname)
	}
	messages := []string{}
...
func OpenPackage(L *LState) int {
	...
	L.SetField(packagemod, "path", LString(loGetPath(LuaPath, MilkPathDefault)))

	L.SetField(packagemod, "config", LString(MilkDirSep+"\n"+MilkPathSep+
		"\n"+MilkPathMark+"\n"+MilkExecDir+"\n"+MilkIgMark+"\n"))
...
func loLoaderPreload(L *LState) int {
	...
	preload := L.GetField(L.GetField(L.Get(EnvironIndex), "pkglib"), "preload")
	if _, ok := preload.(*LTable); !ok {
		L.RaiseError("pkglib.preload must be a table")
	}
	lv := L.GetField(preload, name)
	if lv == LNil {
		L.Push(LString(fmt.Sprintf("no field pkglib.preload['%s']", name)))
		return 1
	}
	L.Push(lv)
	return 1
