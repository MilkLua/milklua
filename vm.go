...
func mainLoopWithContext(L *LState, baseframe *callFrame) {
	...
	for {
		...
		select {
		case <-L.ctx.Done():
			L.RaiseError("context error: %v", L.ctx.Err().Error())
			return
		default:
			if jumpTable[int(inst>>26)](L, inst, baseframe) == 1 {
				return
	...
	jumpTable = [opCodeMax + 1]instFunc{
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_GETTABLE
			...
			if lstr, ok := obj.(LString); ok {
				if lnum, ok := key.(LNumber); ok {
					index := int(lnum) - 1
					runeSlice := []rune(lstr)
					if index >= 0 && index < len(runeSlice) {
						v = LString(string(runeSlice[index]))
					} else {
						v = LNil
					}
				} else {
					v = LNil // if key is not a number, return nil
				}
				// this section is inlined by go-inline
				// source function is 'func (rg *registry) Set(regi int, vali LValue) ' in '_state.go'
				{
		...
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_LE
			...
			if v1, ok1 := lhs.(LNumber); ok1 {
				...
				case LTString:
					...
					ret = strings.Compare(string(lhs.(LString)), string(rhs.(LString))) <= 0
				default:
		...
		opBitwise, // OP_SHL
		opBitwise, // OP_SHR
		func(L *LState, inst uint32, baseframe *callFrame) int { //OP_TYPEASSERT
			reg := L.reg
			cf := L.currentFrame
			lbase := cf.LocalBase
			A := int(inst>>18) & 0xff //GETA
			...
			var defaultValue LValue

			switch C {
			case 0: //bool
				_, typeCheckOK = v.(LBool)
				defaultValue = LNil
			case 1: //number
				num, ok := v.(LNumber)
				typeCheckOK = ok
			...
			case 3: //table
				_, typeCheckOK = v.(*LTable)
				defaultValue = LNil
			case 4: //function
				_, typeCheckOK = v.(*LFunction)
				defaultValue = LNil
			case 5: //userdata
				_, typeCheckOK = v.(*LUserData)
				defaultValue = LNil
			case 6: //thread
				_, typeCheckOK = v.(*LState)
				defaultValue = LNil
			case 7: //channel
				_, typeCheckOK = v.(*LChannel)
				defaultValue = LNil
			default:
				panic("invalid type in type assertion")
			}
...
func opBitwise(L *LState, inst uint32, _ *callFrame) int {
	...
	if !ok1 || !ok2 {
		L.RaiseError("attempt to perform bitwise operation on non-number value")
	}

	var result LNumber
	switch opcode := int(inst >> 26); opcode {
	case OP_BAND:
		result = LNumber(int64(v1) & int64(v2))
	case OP_BOR:
		result = LNumber(int64(v1) | int64(v2))
	case OP_SHL:
		result = LNumber(int64(v1) << uint64(v2))
	case OP_SHR:
		result = LNumber(int64(v1) >> uint64(v2))
	}

	reg.SetNumber(RA, result)
	return 0
}
...
func numberArith(_ *LState, opcode int, lhs, rhs LNumber) LNumber {
	...
	panic("arithmetic operation not supported")
}
...
func objectArith(L *LState, opcode int, lhs, rhs LValue) LValue {
	...
	L.RaiseError("%s", fmt.Sprintf("cannot perform %v operation between %v and %v",
