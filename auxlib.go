...
func (ls *LState) TypeError(n int, typ LValueType) {
	ls.RaiseError("bad argument #%v to %v (%v expected, got %v)",
		n,
		ls.rawFrameFuncName(ls.currentFrame),
		typ.String(),
		ls.Get(n).Type().String(),
	)
}
...
func (ls *LState) PreloadModule(name string, loader LGFunction) {
	preload := ls.GetField(ls.GetField(ls.Get(EnvironIndex), "pkglib"), "preload")
	if _, ok := preload.(*LTable); !ok {
		ls.RaiseError("pkglib.preload must be a table")
	}
...
// If the given index is a LChannel, returns this channel.
// If this argument is absent or is nil, returns ch. Otherwise, raises an error.
