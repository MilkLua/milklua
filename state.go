...
type autoGrowingCallFrameStack struct {
	...
	segIdx   segIdx
	// segSp is the number of frames in the current segment which are used.
	// Full 'sp' value is segIdx * FramesPerSegment + segSp.
...
// `limit` specifies the maximum end range that can be copied from.
// If it's set to -1, then it defaults to stopping at
// the top of the registry (values beyond the top are not initialized,
// so if specifying an alternative `limit` you should
// pass a value <= rg.top.
// If start+n is beyond the limit, then nil values will be copied to the destination slots.
// After the copy, the registry is truncated to be at the end of the copied range,
// ie the original of the copied values
...
func NewState(opts ...Options) *LState {
	...
	// if no options are set, use the default values
	if len(opts) == 0 {
		ls := newLState(defaultOpts)
		ls.OpenLibs()
		return ls
	}
	...
	// if the options are not set, use the default values
	if opt.CallStackSize < 1 {
		opt.CallStackSize = CallStackSize
	}
	if opt.RegistrySize < 128 {
		opt.RegistrySize = RegistrySize
	}
	if opt.RegistryMaxSize < opt.RegistrySize {
		opt.RegistryMaxSize = 0 // 0 means no limit
	} else if opt.RegistryGrowStep < 1 {
	...
	// open the libraries if the option is not set
	if !opt.SkipOpenLibs {
		ls.OpenLibs()
	}
...
// If the original state has context.Context,
// the new state has a new child context of the original state
// and this function returns its cancel function.
...
func (ls *LState) PCall(nargs, nret int, errfunc *LFunction) (err error) {
	...
	captureStackTrace := func(includeNewline bool) string {
		buf := make([]byte, 4096)
		runtime.Stack(buf, false)
		trace := strings.Trim(string(buf), "\000")
		if includeNewline {
			return trace + "\n" + ls.stackTrace(0)
		}
		return trace + ls.stackTrace(0)
	}
	...
	defer func() {
		...
		if r := recover(); r != nil {
			var apiErr *ApiError
			// if the panic is not an ApiError, create a new ApiError
			if errApi, ok := r.(*ApiError); !ok {
				apiErr = newApiErrorS(ApiErrorPanic, fmt.Sprint(r))
				if ls.Options.IncludeGoStackTrace {
					apiErr.StackTrace = captureStackTrace(true)
				}
			} else {
				apiErr = errApi
			}

			// if there is an error function, call it
			if errfunc != nil {
				ls.Push(errfunc)
				ls.Push(apiErr.Object)
				ls.Panic = panicWithoutTraceback
				// use defer to catch the panic
				defer func() {
					ls.Panic = oldPanic
					if r2 := recover(); r2 != nil {
						if errApi, ok := r2.(*ApiError); !ok {
							apiErr = newApiErrorS(ApiErrorPanic, fmt.Sprint(r2))
							if ls.Options.IncludeGoStackTrace {
								apiErr.StackTrace = captureStackTrace(false)
							}
						} else {
							apiErr = errApi
							apiErr.StackTrace = ls.stackTrace(0)
						}
						resetState()
					}
				}()
				ls.Call(1, 1)
				apiErr = newApiError(ApiErrorError, ls.Get(-1))
			} else if len(apiErr.StackTrace) == 0 {
				apiErr.StackTrace = ls.stackTrace(0)
			}
			resetState()
			err = apiErr
